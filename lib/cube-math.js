...
export function projectVertex(x, y, z, rx, ry, centerX, centerY) {
  const P = 1200;
  const rxr = rx * Math.PI / 180;
  const ryr = ry * Math.PI / 180;
  const cx = Math.cos(rxr), sx = Math.sin(rxr);
...
export function computeWireframe(rx, ry, zoomedFace) {
  const H = 150;
  const vertices = [
...
export function findClickedFace(px, py, centerX, centerY, rxDeg, ryDeg) {
  const P = 1200, H = 150;
  const rx = rxDeg * Math.PI / 180;
  const ry = ryDeg * Math.PI / 180;
  const cX = Math.cos(rx), sX = Math.sin(rx);
